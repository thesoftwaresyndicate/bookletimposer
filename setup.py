...
class build_man(distutils.cmd.Command):
    ...
    user_options= [('man_sources', None, 'list of man sources in the source tree')]
    man_build_dir = os.path.join('build', 'man')
...
class clean_man(distutils.command.clean.clean):
    ...
    def run(self):
        self.man_build_dir = os.path.join("build", "man")
        if os.path.isdir(self.man_build_dir):
            remove_tree(self.man_build_dir)
